...
void PlayGame::spawn(Blank *map) {
    for (int row = 0; row < 25; row++) {
        for (int col = 0; col < 79; col++) {
            ...
            if (letter == '0') {
                Item * potion = ItemFactory::createItem(ItemFactory::Type::POTION_RH, d->picture(), );
                d->picture() = potion;
            } else if (letter == '1') {
                Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_BA, d->picture(), );
                d->picture() = potion;
            } else if (letter == '2') {
                Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_BD, d->picture(), );
                d->picture() = potion;
            } else if (letter == '3') {
                Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_PH, d->picture(), );
                d->picture() = potion;
            } else if (letter == '4') {
                Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_WA, d->picture(), );
                d->picture() = potion;
            } else if (letter == '5') {
                Item * potion = ItemFactory::createItem(ItemFactory::Type::POTION_WD, d->picture(), );
                d->picture() = potion;
            } 
            else if (name == 'H') {
                Enemy *hp = new Human(d->picture(), ); 
                d->picture() = hp;  
                eVec.emplace_back(hp);
            } else if (name == 'W') {
                Enemy *wp = new Dwarf(d->picture(), ); 
                d->picture() = wp; 
                eVec.emplace_back(wp);
            } else if (name == 'E') {
                Enemy *ep = new Elf(d->picture(),); 
                d->picture() = ep; 
                eVec.emplace_back(ep);
            } else if (name == 'O') {
                Enemy *op = new Orc(d->picture(), ); 
                d->picture() = op; 
                eVec.emplace_back(op);
            } else if (name == 'M') {
                Enemy *mp = new Merchant(d->picture(), ); 
                d->picture() = mp; 
                eVec.emplace_back(mp);
            } else {
                Enemy *lp = new Halfling(d->picture(), ); 
                d->picture() = lp; 

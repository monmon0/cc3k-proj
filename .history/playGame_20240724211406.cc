...
void PlayGame::spawn(std::string s) {
    ...
    for(int i = 0; i < s.length(); i++) {
        ...
        if (letter == '0') {
            Item * potion = ItemFactory::createItem(ItemFactory::Type::POTION_RH, d->picture(), x, y);
            d->picture() = potion;
        } else if (letter == '1') {
            Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_BA, d->picture(), x, y);
            d->picture() = potion;
        } else if (letter == '2') {
            Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_BD, d->picture(), x, y);
            d->picture() = potion;
        } else if (letter == '3') {
            Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_PH, d->picture(), x, y);
            d->picture() = potion;
        } else if (letter == '4') {
            Item * potion  = ItemFactory::createItem(ItemFactory::Type::POTION_WA, d->picture(), x, y);
            d->picture() = potion;
        } else if (letter == '5') {
            Item * potion = ItemFactory::createItem(ItemFactory::Type::POTION_WD, d->picture(), x, y);
            d->picture() = potion;
        } else if (letter == '6') {
            Item *treasure = ItemFactory::createItem(ItemFactory::Type::GOLD_NORMAL, d->picture(), x, y);
            d->picture() = treasure;
        } else if (letter == '7') {
            Item *treasure = ItemFactory::createItem(ItemFactory::Type::GOLD_SMALL, d->picture(), x, y);
            d->picture() = treasure;
        } else if (letter == '8') {
            Item *treasure = ItemFactory::createItem(ItemFactory::Type::GOLD_DRAGON, d->picture(), x, y);
            d->picture() = treasure;
            ...
            Enemy *hp = new Human(d->picture(), x, y); 
            d->picture() = hp;  
            eVec.emplace_back(hp);
        } else if (letter == 'W') {
            Enemy *wp = new Dwarf(d->picture(), x, y); 
            d->picture() = wp; 
            eVec.emplace_back(wp);
        } else if (letter == 'E') {
            Enemy *ep = new Elf(d->picture(), x, y); 
            d->picture() = ep; 
            eVec.emplace_back(ep);
        } else if (letter == 'O') {
            Enemy *op = new Orc(d->picture(), x, y); 
            d->picture() = op; 
            eVec.emplace_back(op);
        } else if (letter == 'M') {
            Enemy *mp = new Merchant(d->picture(), x, y); 
            d->picture() = mp; 
            eVec.emplace_back(mp);
        } else {
            Enemy *lp = new Halfling(d->picture(), x, y); 
            d->picture() = lp; 

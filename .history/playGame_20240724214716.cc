    ...
    for (int row = 0; row < 25; row++) {
        for (int col = 0; col < 79; col++) {
            ...
            if (letter == '0') {
                ...
                if (map->charAt(row - 1, col - 1) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row - 1, col) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row, col - 1) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row + 1, col + 1) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row + 1, col) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row, col) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row, col) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                else if (map->charAt(row, col) == 'D') Enemy *dp = new Dragon(d->picture(), col, row, static_cast<Dragon_Hoard*>(item));
                d->picture() = dp;

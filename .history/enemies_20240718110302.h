...
class Enemy : public Decorator, public Subject {
    ...
    public:
        Enemy(char id, AsciiArt *next, int x, int y, int hp, int atk, int def)
            : ID Decorator(next), xCoord(x), yCoord(y), hp(hp), atk(atk), def(def) {} 
        virtual ~Enemy() {}
...
Human::Human(AsciiArt *next, int xCoord, int yCoord)
    : Enemy('H', next, xCoord, yCoord, 140, 20, 20) {

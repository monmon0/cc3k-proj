...
void Enemy::atkOrMv(Player *pc, AsciiArt *curr) {
    std::vector<int> arr = {-1, 0, 1};
    
    int xDiff = pc->getX() - x; 
    int yDiff = pc->getY() - y; 
    int *a1 = std::find(std::begin(array), std::end(array), xDiff);
    int *a2 = std::find(std::begin(array), std::end(array), yDiff);
    if (a1 != std::end(array) && a2 != std::end(array)) {
        // attack pc 
    } else {
        while (! fPressed) {

...
bool comparePairs(const std::pair<int, std::pair<int, int>>& a, const std::pair<int, std::pair<int, int>>& b) {
...
void PlayGame::attackOrMove() {
    ...
    for (auto it = eMap.begin(); it != eMap.end(); it++) { 
        eVec[it->first]->atkOrMv(p, d); 
        d->setAction(eVec[it->first]->getAnnouncement());
        it->second = make_pair(eVec[it->first]->getX(), eVec[it->first]->getY()); t
    }
}
